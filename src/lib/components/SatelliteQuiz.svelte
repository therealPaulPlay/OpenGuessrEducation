<script>
	...
	async function fetchRegions() {
		try {
			...
			const response = await fetch(`/json/satellite/${jsonName}.json`);
	...
	function setQuizContent() {
		...
		const otherLocations = shuffleArray(locationsArray.filter((loc) => loc.name !== currentLocation.name)).slice(0, 3);
	...
	function updateMap(lat, long) {
		...
		const boxHeight = 0.2;
		currentExtent = [long - boxWidth / 2, lat - boxHeight / 2, long + boxWidth / 2, lat + boxHeight / 2];
		...
		const centerConstraint = function () {
			...
			if (
				...
				const constrainedLonLat = constrainCenter([longitude, latitude], currentExtent);
