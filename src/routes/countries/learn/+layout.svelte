<script>
    ...
    async function fetchCountryCodes() {
        ...
        countryCodes = await fetch("/json/countryCodes.json").then((response) =>
            response.json(),
        );
    }
    ...
    async function fetchJsonData() {
        ...
        try {
            telephonePrefixes = await fetch(
                "/json/country-data/telephone-prefixes.json",
            ).then((response) => response.json());
            topLevelDomains = await fetch(
                "/json/country-data/country-tld.json",
            ).then((response) => response.json());
            countryLanguages = await fetch(
                "/json/country-data/country-languages.json",
            ).then((response) => response.json());
            populationData = await fetch(
                "/json/country-data/population-data.json",
            ).then((response) => response.json());
            countryGDP = await fetch(
                "/json/country-data/country-gdp.json",
            ).then((response) => response.json());
            videoSources = await fetch(
                "/json/country-data/country-videos.json",
            ).then((response) =>
    ...
    onMount(async () => {
        countryNameFromPath = $page.url.pathname
            .replace("/countries/learn/", "")
            .replaceAll("-", " ");
    ...
    $effect(() => {
        ...
        childHasNoContent =
            [...childrenContainer.childNodes].find(
...
<article class="container mx-auto p-6 px-1 max-w-3xl">
    ...
    <div
        class="w-full bg-base-300 rounded-lg flex flex-wrap items-center p-2 gap-2 mt-2">
        {#if topLevelDomains}
            <div class="stat-pill">
                <EthernetPort />
        ...
        <a
            ...
            href="https://openguessr.com/?play-map={countryName.replaceAll(
                ' ',
                '_',
            )}"
            target="_blank">
